...
使用的数据包括如下几个部分：
\begin{enumerate}
	\item 总得分 (\text{total\_scores})
	\item 比赛轮次 (\text{rounds\_per\_match})
	\item 最大连续得分 (\text{max\_consecutive\_points})
	\item 赛点次数 (\text{game\_points})
	\item 胜负情况 (\text{victory\_labels})
	\end{enumerate}
